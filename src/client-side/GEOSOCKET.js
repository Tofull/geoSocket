var GEOSOCKET = (function(geosocket) {
	...
	geosocket.emitMessage = function(message) {
		_this._socket.emit('message', {
			message: message
		});
	}

	geosocket._onMessageReceive = function(callback) {
		callback = callback || function() {};
		_this._socket.on('messageReceive', function(data) {
			callback(data)
		});
	}

	geosocket.onMessageReceive = function(callback) {
		_this._socket.off('messageReceive');
		_this._onMessageReceive(callback);
	}

	geosocket._onCount = function(callback) {
		callback = callback || function() {};
		_this._socket.on('count', function(data) {
			callback(data)
		});
	}

	geosocket.onCount = function(callback) {
		_this._socket.off('count');
		_this._onCount(callback);
	}
